...
const entitySchema = new mongoose.Schema({
  ...
  parentName: { type: String, default: null }, // Optional field for hierarchy
...
app.post('/api/entities', async (req, res) => {
  try {
    ...
    if (parentName) {
      ...
      const parentEntity = await Entity.findOne({ name: parentName });
...
app.get('/api/entities', async (req, res) => {
  try {
    const entities = await Entity.find({ parentName: null }).populate({
      path: 'children',
      populate: {
        path: 'children',
        populate: {
    ...
    for (const entityData of updatedEntities) {
      ...
      if (entity) {
        ...
        const newEntity = new Entity({
          ...
          parentName: entityData.parentName || null,
...
app.delete('/api/entities/:id', async (req, res) => {
  try {
    ...
    const deleteChildren = async (entityId) => {
      const children = await Entity.find({ parentName: entityId });
...
app.post('/api/exam', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error saving exam data', error: error.message });
  }
});
...
app.put('/api/exams/:id', async (req, res) => {
  ...
  if (!examName || !examName.trim()) {
    return res.status(400).json({ message: 'Exam name is required and cannot be empty.' });
  }
...
app.post("/api/tests/markCompleted", authenticate, async (req, res) => {
  ...
  if (!email || !testName || !submittedTest) {
    return res.status(400).json({ message: "Email and testName are required." });
  }
...
app.put("/users/update-avatar", upload.single("avatar"), async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(userId, { profileImageUrl: avatarUrl }, { new: true });
...
app.get('/users', authenticate, async (req, res) => {
  ...
  try {
    ...
    const userExam = user.completedTests.find(test => test.examId === examId) || 
...
app.put('/users/update', authenticate, async (req, res) => {
  ...
  const { firstName, lastName, email, phone, institutionName, institutionType, class: userClass, degreeName, passingYear, universityName } = req.body;
  try {
    ...
    return res.status(200).json({ message: 'User information updated successfully', data: user });
  } catch (error) {
